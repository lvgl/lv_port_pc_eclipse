...
int main (void)
{
    ...
    /* Initialize a phantom application configuration and run the app.
     * The phantom app. will send data to the real application*/
    lv_app_phantom_conf_t conf;
    conf.com_listen = phantom_listen;
    lv_app_inst_t * app_phantom = lv_app_run(lv_app_dsc_get("Phantom"), &conf);
    lv_app_rename(app_phantom, "My phantom");
